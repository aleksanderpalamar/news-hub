...
export function NewsCard({ article }: { article: Article }) {
    ...
    const [imgSrc, setImgSrc] = useState(urlToImage || "/placeholder.svg?height=192&width=384")
    ...
    return (
        <Card className="overflow-hidden h-full flex flex-col hover:shadow-lg transition-shadow relative">
            ...
            <CardContent className="py-4 flex-grow">
                ...
                <p className="text-gray-600 line-clamp-3">{description || "Sem descrição disponível"}</p>
            </CardContent>
            <CardFooter className="flex justify-between items-center pt-0 pb-4 text-sm text-gray-500">
                <span className="font-medium">{source?.name || "Fonte desconhecida"}</span>
                <time dateTime={publishedAt}>{formattedDate}</time>
            </CardFooter>
            <a href={url} target="_blank" rel="noopener noreferrer" className="absolute inset-0 z-10" aria-label={title}>
